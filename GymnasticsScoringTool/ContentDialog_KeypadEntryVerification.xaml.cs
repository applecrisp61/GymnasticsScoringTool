...
namespace GymnasticsScoringTool {
    public sealed partial class ContentDialog_KeypadEntryVerification : ContentDialog {

        // private bool _updateCancelButtonsChanged = false;

        public ContentDialog_KeypadEntryVerification() {
            this.InitializeComponent();

            this.Loaded += ContentDialog_KeypadEntryVerification_LoadedFormat;
            this.Loaded += ContentDialog_LoadedSetUpdateCancelButtons;

            this.Opened += ContentDialog_KeypadEntryVerification_OpenedLoadGymnasts;
        }

        private void ContentDialog_KeypadEntryVerification_LoadedFormat(object sender, RoutedEventArgs e) {
            string teamNameDisplay = "";

            var teamList = Meet.TeamNamesWithUpdatedGymnasts();
            int stringLen = 0;
            foreach (var t in teamList) {
                stringLen += t.Length;
            }

            if ((stringLen < 40) || (teamList.Count == 1)) {
                int commaCheck = 0;
                foreach (var teamName in teamList) {
                    if (commaCheck > 0) teamNameDisplay += ", ";
                ...
                teamNameDisplay = "Multiple teams";
            }
        ...
        private void ContentDialog_KeypadEntryVerification_OpenedLoadGymnasts(ContentDialog sender, ContentDialogOpenedEventArgs args) {
            ContentDialog cd = sender as ContentDialog;
            if (cd == null) {
                throw new Exception("EXCEPTION >> Unexpected type for sender on ContentDialog loaded event; expected ContentDialog; actual " + sender.GetType().Name);
            }
